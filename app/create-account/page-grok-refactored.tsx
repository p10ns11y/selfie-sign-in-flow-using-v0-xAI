...
import { createAccountMachine } from "@/machines/createAccountMachine"
import { REQUIRED_ANGLES } from "@/utils/cameraUtils"
import { InfoView } from "@/components/createaccount/InfoView"
import { CreateCaptureView } from "@/components/createaccount/CreateCaptureView"
import { CompleteView } from "@/components/createaccount/CompleteView"
...
export default function CreateAccountPage({ onBack }: CreateAccountPageProps) {
  ...
  const [state, send] = useMachine(createAccountMachine, { actions })
  ...
  const handleRetake = () => send("RETAKE")
  ...
  if (state.matches("complete") || state.matches("submitting")) {
    return <CompleteView onSubmit={() => send("SUBMIT")} isCapturing={state.context.isCapturing} />
  }
